...
ボタンを押すと、本人だけに理由カテゴリのセレクトメニューを表示します。選択後、発動者が参加しているVCのチャットへ、Bot名義でカテゴリだけを匿名投稿し、そのVCの参加者全員へ個別メンションします。発動者がVCに参加していない場合は投稿しません。参加者のミュートや非公開ログへの記録は行いません。
...
投稿された⏱と✕のボタンを、VCへ参加した状態で押して動作を確認してください。通常の通知はカード設置チャンネルではなく、参加中VCのチャットへ投稿されます。通知には `@everyone` ではなく、確認時点のVC参加者だけを個別メンションします。X発動直前にも同じ確認を行うため、設置後にロール設定が変わった場合は誰もミュートせず公開警告を投稿します。
